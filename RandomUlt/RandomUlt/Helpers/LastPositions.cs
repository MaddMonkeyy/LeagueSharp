...
namespace RandomUlt.Helpers
{
    internal class LastPositions
    {
        ...
        public LastPositions(Menu config)
        {
            ...
            if (SupportedChamps())
            {
                config.AddItem(new MenuItem("UseR", "Dùng R")).SetValue(true);
                if (player.ChampionName == "Gangplank")
                {
                    config.AddItem(new MenuItem("gpWaves", "Đợt Ulti Gangplank gây sát thương")).SetValue(new Slider(2, 1, 7));
                }
                if (player.ChampionName == "Xerath")
                {
                    config.AddItem(new MenuItem("XerathUlts", "Xerath Ulti gây sát thương")).SetValue(new Slider(2, 1, 3));
                }
                if (player.ChampionName == "Draven")
                {
                    config.AddItem(new MenuItem("Backdamage", "Đếm cú đánh thứ 2")).SetValue(true);
                    config.AddItem(new MenuItem("CallBack", "Giảm thời gian giữa các cú đánh")).SetValue(true);
                }
                config.AddItem(new MenuItem("Hitchance", "Độ chính xác")).SetValue(new Slider(3, 1, 5));
                Menu DontUlt = new Menu("Không Ulti", "DontUltRandomUlt");
                ...
                config.AddItem(new MenuItem("Alliesrange", "Khoảng cách thấp nhất của đồng minh với mục tiêu"))
                    .SetValue(new Slider(1500, 500, 2000));
                config.AddItem(new MenuItem("EnemiesAroundYou", "Không bắn nếu kẻ địch trong phạm vi"))
                    .SetValue(new Slider(600, 0, 2000));
                config.AddItem(new MenuItem("waitBeforeUlt", "Thời gian chờ trước khi Ulti(ms)"))
                    .SetValue(new Slider(600, 0, 3000));
                config.AddItem(new MenuItem("BaseUltFirst", "Ưu Tiên BaseUlt")).SetValue(false);
                config.AddItem(new MenuItem("Collision", "Tính toán số dame bị tổn thất")).SetValue(true);
                config.AddItem(new MenuItem("drawNotification", "Hiển Thị Thông Báo")).SetValue(true);
            }
            config.AddItem(new MenuItem("RandomUltDrawings", "Hiển thị địa điểm khả thi")).SetValue(false);
            Menu orbBlock = new Menu("Nút không bắn", "BlockKeys");
            orbBlock.AddItem(new MenuItem("OrbBlock1", "Không bắn khi ấn nút"))
                .SetValue(new KeyBind(65, KeyBindType.Press));
            orbBlock.AddItem(new MenuItem("OrbBlock2", "Không bắn khi ấn nút"))
                .SetValue(new KeyBind(88, KeyBindType.Press));
            orbBlock.AddItem(new MenuItem("OrbBlock3", "Không bắn khi ấn nút"))
                .SetValue(new KeyBind(67, KeyBindType.Press));
            orbBlock.AddItem(new MenuItem("ComboBlock", "Không bắn khi Combo"))
                .SetValue(new KeyBind(32, KeyBindType.Press));
            orbBlock.AddItem(new MenuItem("OnlyCombo", "Chỉ có nút Combo")).SetValue(true);
